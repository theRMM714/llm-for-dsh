...
重试与修复是两条互不重叠的路：修复改**请求形状**，重试处理**报文无法解释的间歇性拒绝**。证据是实测出来的——同一份 `gaps 0` 的请求连过三次、第四次被拒。

`src/retries/index.js` 是目录（唯一事实来源）；`src/retries/<id>.js` 是实现。一条规则声明：
...
| `title` / `hint` | 设置页文案 |
| `defaultEnabled` | 无用户分节时的默认值 |
| `matches({ status, bodyText, url })` | 这条响应是否该重试 |

拦截器在状态码 ≥ 400 时读过响应副本（上限 8 KiB）后按目录顺序询问启用的规则；命中即**用同一份字节重发**，次数上限来自设置（默认 2、硬上限 5），退避为 `300ms × 尝试次数` 加抖动。只有可重建的调用会被重试：字符串 body 一定可以，从 clone 读到 body 的 Request 用该文本重建；其余只发一次。

因为重试发生在 HTTP 层、且失败点是工具结果之后的续写请求，重试不会重跑工具，也不会产生重复副作用。
