...
| 选项 | 取值 | 作用 | 默认 |
| --- | --- | --- | --- |
| `recentTurns` | 非负整数，`0` = 不限 | 只对最新的 N 个 assistant 轮注入。注入量按窗口内降级轮数增长，且每一步都重发，所以这是长会话里的主要成本开关 | `0`（不限） |
| `singleReasoningSlot` | 布尔 | 合成项只写 `content: [{ type: "reasoning_text" }]` 一个槽，省掉 summary 里的重复副本；捕获到的原始项则完全按网关发来的形状回放，不再补槽 | `false` |
| `placeholderReasoning` | 布尔 | 某一轮**既无捕获项也无历史思考文本**（提供方只回传了裸工具调用）时，补一个文本为单个空格的思考项，只为满足「必须回传」的存在性检查；空格而非空串，是因为会 trim 或拒空的分支仍然接受空白 | `false` |
