...
| 只回放最近 N 轮思考 | `responses-reasoning-echo` 的选项。`0` = 不限（默认，等于当前行为）；长会话里注入量按窗口内降级轮数增长且每步重发，填 `1` 通常就够，可把它从约 1 MB 降到几 KB |
| 思考项只写一个文本槽 | 同一修复项的选项。默认同时写 `summary` 与 `reasoning_text` 两个槽（保险，但文本翻倍）；开启后合成项只写 `reasoning_text`、捕获到的原始项原样回放，注入文本约减半 |
| 没有思考时补一个占位项 | 同一修复项的选项。提供方某一轮可能**完全不回传思考**（模型只吐一个裸的工具调用），此时没有任何内容可回放；开启后补一个文本为单个空格的思考项，只为满足「必须回传」的存在性检查。默认关闭 |

三个选项互相独立、**可以同时开启**：一个决定「给哪几轮注入」，一个决定「每个注入项写几个槽」，一个决定「什么都没有时是否补占位项」。
