 ...
 * Host's own catalog (`{"fixes":[{"id":"responses-reasoning-echo","title":"思考内容回传（Responses 协议）","hint":"命中 Responses 请求时，把该轮思考项补回 function_call 之前；上一轮响应捕获不到时，用历史里仍保留的思考文本合成。","detail":"第三方网关在 thinking 模式下要求随历史回传上一轮思考内容。DSH 在跨模型、跨适配器或压缩之后会丢掉思考签名，pi-ai 的 Responses 分支只回传带签名的思考项，于是带工具调用的续轮被网关以 400 拒绝。本修复只改写 /responses 请求，且在已有思考项时不动作。","defaultEnabled":false,"options":[{"id":"recentTurns","kind":"number","title":"最近 N 轮写真实思考文本","hint":"每一轮的缺项都会补上；这里只决定哪几轮用真实文本。N=0（默认）表示每一轮都用真实文本；填 1 则只有最新一轮用真实文本，更早的缺项轮改用单空格占位项——覆盖面不变，注入量从约 1 MB 降到几 KB（需要同时开启「没有思考时补一个占位项」）。","min":0},{"id":"placeholderReasoning","kind":"boolean","title":"没有思考时补一个占位项","hint":"提供方某一轮可能完全不回传思考（模型只吐工具调用）。开启后，这类轮次会补一个文本为单个空格的思考项，只为满足「必须回传」的存在性检查；关闭则不改写（默认）。"},{"id":"reasoningTextOnly","kind":"boolean","title":"思考形状统一成最保守形式","hint":"两件事一起做：请求里所有思考项只保留 reasoning_text 内容槽（summary 的文本搬进 content、id 保留），并且去掉请求参数 reasoning 里的 summary（只留 effort）。实测同一中继的两个上游一个要求回传 reasoning_text、另一个对 summary 报 unknown field \"summary\"，这两处都要清掉。默认关闭。"},{"id":"singleReasoningSlot","kind":"boolean","title":"思考项只写一个文本槽","hint":"默认同时写 summary 与 reasoning_text（保险，但文本翻倍）。开启后合成项只写 reasoning_text，捕获到的原始项原样回放，注入文本约减半。"}]}],"retries":[{"id":"reasoning-text-not-passed-back","title":"思考内容未回传（Responses 思考模式）","hint":"网关返回 400 且正文同时含 reasoning_text 与 must be passed back 时重试。实测同一份请求会时而通过、时而失败，属于网关侧的间歇行为，报文结构本身没有问题。","defaultEnabled":false}],"defaults":{"enabled":[],"hosts":[],"diagnostics":false,"recentTurns":0,"singleReasoningSlot":false,"placeholderReasoning":false,"retries":[],"retryAttempts":2}}`), so the page and the Host enforce
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    try {
       ...
       */
      const CATALOG = {"fixes":[{"id":"responses-reasoning-echo","title":"思考内容回传（Responses 协议）","hint":"命中 Responses 请求时，把该轮思考项补回 function_call 之前；上一轮响应捕获不到时，用历史里仍保留的思考文本合成。","detail":"第三方网关在 thinking 模式下要求随历史回传上一轮思考内容。DSH 在跨模型、跨适配器或压缩之后会丢掉思考签名，pi-ai 的 Responses 分支只回传带签名的思考项，于是带工具调用的续轮被网关以 400 拒绝。本修复只改写 /responses 请求，且在已有思考项时不动作。","defaultEnabled":false,"options":[{"id":"recentTurns","kind":"number","title":"最近 N 轮写真实思考文本","hint":"每一轮的缺项都会补上；这里只决定哪几轮用真实文本。N=0（默认）表示每一轮都用真实文本；填 1 则只有最新一轮用真实文本，更早的缺项轮改用单空格占位项——覆盖面不变，注入量从约 1 MB 降到几 KB（需要同时开启「没有思考时补一个占位项」）。","min":0},{"id":"placeholderReasoning","kind":"boolean","title":"没有思考时补一个占位项","hint":"提供方某一轮可能完全不回传思考（模型只吐工具调用）。开启后，这类轮次会补一个文本为单个空格的思考项，只为满足「必须回传」的存在性检查；关闭则不改写（默认）。"},{"id":"reasoningTextOnly","kind":"boolean","title":"思考形状统一成最保守形式","hint":"两件事一起做：请求里所有思考项只保留 reasoning_text 内容槽（summary 的文本搬进 content、id 保留），并且去掉请求参数 reasoning 里的 summary（只留 effort）。实测同一中继的两个上游一个要求回传 reasoning_text、另一个对 summary 报 unknown field \"summary\"，这两处都要清掉。默认关闭。"},{"id":"singleReasoningSlot","kind":"boolean","title":"思考项只写一个文本槽","hint":"默认同时写 summary 与 reasoning_text（保险，但文本翻倍）。开启后合成项只写 reasoning_text，捕获到的原始项原样回放，注入文本约减半。"}]}],"retries":[{"id":"reasoning-text-not-passed-back","title":"思考内容未回传（Responses 思考模式）","hint":"网关返回 400 且正文同时含 reasoning_text 与 must be passed back 时重试。实测同一份请求会时而通过、时而失败，属于网关侧的间歇行为，报文结构本身没有问题。","defaultEnabled":false}],"defaults":{"enabled":[],"hosts":[],"diagnostics":false,"recentTurns":0,"singleReasoningSlot":false,"placeholderReasoning":false,"retries":[],"retryAttempts":2}}

      /** This bundle's build id; a stale page can otherwise make a fixed bug look present. */
      const BUILD = "c9283e9a7660"

      /** Same-origin route the Host half serves the log paths and the clear action on. */
      const LOG_ROUTE = "/llm-compat/log"
      ...
      function CompatSettingsPage(props) {
        ...
        /** The log files the Host reported, and the outcome of the last clear. */
        const [logPaths, setLogPaths] = React.useState(null)
        const [logAction, setLogAction] = React.useState(null)

        React.useEffect(() => {
          let live = true
          Promise.resolve()
            .then(() => fetch(LOG_ROUTE, { headers: { accept: 'application/json' } }))
            ...
            .catch(() => {
              // A missing route only means the running Host predates this page.
              if (live) setLogPaths(null)
            })
          return () => {
            live = false
          }
        ...
        const clearLog = () => {
          setLogAction({ state: 'loading' })
          Promise.resolve()
            .then(() => fetch(LOG_ROUTE + '?action=clear', { headers: { accept: 'application/json' } }))
            .then((response) => response.text().then((text) => ({ status: response.status, text })))
            .then((answer) => {
              if (answer.text.trim().length === 0) {
                throw new Error('Host 侧没有这个路由（运行中的 Host 可能是重启前的版本），请重启 dsh web 后再试。')
              }
              const body = JSON.parse(answer.text)
              if (typeof body.error === 'string') throw new Error(body.error)
              setLogAction({ state: 'done', body })
            })
            .catch((error) => setLogAction({
              state: 'failed',
              message: error !== null && typeof error === 'object' && typeof error.message === 'string' ? error.message : String(error),
            }))
        }
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  ...
                  { className: 'llm-compat-hint' },
                  logPaths === null
                    ? '开启后把每次改写与响应状态写进 Host 侧的 llm-compat.log；被网关拒绝时，当次请求体会写进同目录的 llm-compat-rejected.jsonl。'
                    : '改写与响应状态：' + logPaths.log + '；被拒请求体：' + logPaths.rejected + '。',
                ),
              ),
            ),
            React.createElement(
              'div',
              { className: 'llm-compat-actions' },
              React.createElement(
                'button',
                { type: 'button', className: 'llm-compat-button', onClick: clearLog, disabled: logAction !== null && logAction.state === 'loading' },
                '清理日志',
              ),
              logAction === null
                ? null
                : React.createElement(
                  ...
                  logAction.state === 'loading'
                    ? '正在清理…'
                    : logAction.state === 'done'
                      ? (logAction.body.cleared.length === 0 ? '没有可清空的文件。' : '已清空：' + logAction.body.cleared.join('、'))
                      : '清理失败：' + logAction.message,
                ),
            ),
          ),
      ...
      function apply(ctx) {
        ...
        slots.inject('settings.section', () =>
          slots.register(
            { name: 'settings.section', id: 'llm-compat', order: 31, label: () => 'LLM 工具' },
      ...
      const CSS = [
        ...
        '.llm-compat-actions{display:flex;align-items:center;flex-wrap:wrap;gap:10px}',
        '.llm-compat-button{appearance:none;padding:6px 12px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-1);border:.5px solid var(--dsw-alias-border-l1);border-radius:8px;font:inherit;font-size:12.5px;line-height:18px;cursor:pointer}',
        '.llm-compat-button:hover:not(:disabled){background:var(--dsw-alias-bg-layer-2)}',
        '.llm-compat-button:disabled{opacity:.6;cursor:default}',
