...
export function createDiagnosticLog(options = {}) {
  ...
  return {
    ...
    /**
     * The two file paths, so a surface can name them without restating them.
     * @returns `{ log, rejected }`.
     */
    paths() {
      return { log: path, rejected: rejectedPath }
    },
    /**
     * Empty both files, so the operator can start a fresh observation without
     * hunting for the paths. Failures are swallowed like every other write here:
     * a log the process cannot clear must not fail the request that asked.
     * @returns the paths that were actually emptied.
     */
    clear() {
      const cleared = []
      for (const target of [path, rejectedPath]) {
        try {
          writeFileSync(target, '')
          cleared.push(target)
        } catch {
          // Reported by omission: the caller answers with what it could clear.
        }
      }
      return cleared
    },
